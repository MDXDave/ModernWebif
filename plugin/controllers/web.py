...
from models.services import getCurrentService, getBouquets, getServices, getSubServices, getChannels, getSatellites, \
    getBouquetEpg, getBouquetNowNextEpg, getSearchEpg, getChannelEpg, getNowNextEpg, getSearchSimilarEpg, \
    getAllServices, getPlayableServices, getPlayableService, getParentalControlList, getEvent, loadEpg, saveEpg
...
from models.timers import getTimers, addTimer, addTimerByEventId, editTimer, removeTimer, toggleTimerStatus, \
    cleanupTimer, writeTimerList, recordNow, tvbrowser, getSleepTimer, setSleepTimer, getPowerTimer, setPowerTimer, \
    getVPSChannels
...
from models.config import getSettings, addCollapsedMenu, removeCollapsedMenu, setRemoteGrabScreenshot, setZapStream, \
    saveConfig, getZapStream, setEPGSearchType
from models.stream import getStream, getTS, getStreamSubservices
from models.servicelist import reloadServicesLists
from models.mediaplayer import mediaPlayerAdd, mediaPlayerRemove, mediaPlayerPlay, mediaPlayerCommand, \
    mediaPlayerCurrent, mediaPlayerList, mediaPlayerLoad, mediaPlayerSave, mediaPlayerFindFile
    ...
    def P_powertimer(self, request):
        if len(request.args):
            res = self.testMandatoryArguments(request,
                                              ["start", "end", "timertype", "repeated", "afterevent", "disabled"])
            if res:
                return res
